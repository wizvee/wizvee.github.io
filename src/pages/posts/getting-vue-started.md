...
각 뷰 인스턴스는 생성될 때 일련의 초기화 단계를 거칩니다. 그 과정에서 사용자 정의 로직을 실행할 수 있는 **라이프사이클 훅**도 호출됩니다. 라이프사이클 훅은 총 8개가 있습니다.
...
객체지향 프로그래밍에는 **SOLID 설계 원칙**이 있듯, 컴포넌트 설계에는 **FIRST 설계 원칙**이 있습니다. 물론 이를 지켜가며 컴포넌트를 설계하기란 어렵겠지만, 최대한 따라가며 설계하도록 노력하는 것이 좋다고 합니다.

1. **Focused**: SOLID 설계 원칙에서 **단일 책임 원칙**과 같습니다. 커다란 하나를 바로 만드는 것이 아닌, 각각의 기능을 가진 작은 부품들을 만들어서 커다란 하나로 조립해 간다는 것입니ㅏㄷ.

2. **Independent**: 컴포넌트가 자기가 가지고 있는 기능을 수행하기 위해서 다른 컴포넌트의 도움을 받지 않아야 한다는 것을 의미합니다.

3. **Reusable**: 컴포넌트는 재사용 가능해야 합니다. 예를 들어 이메일 입력 텍스트 박스와 비밀번호 입력 텍스트 박스를 합친 로그인 폼 컴포넌트가 있다면, 이는 로그인 상황에서만 사용할 수 있는 컴포넌트가 됩니다. 하지만 이메일 입력 텍스트 박스, 비밀번호 입력 텍스트 박스를 따로 컴포넌트로 작성한다면 이메일을 입력받는 상황 혹은 비밀번호를 입력받는 상황 모두에 재사용이 가능합니다.

4. **Small**: 컴포넌트는 가능한 작고 단순한 기능을, 즉 작은 API를 가져야 함을 의미합니다.

5. **Testable**: 쉬운 디버깅이 가능한 컴포넌트를 작성해야 한다는 것을 의미합니다.

### 범위 컴파일

범위 컴파일은 외부에서 주입받는 템플릿입니다. 범위 컴파일은 뷰에서 제공해주는 내장 컴포넌트인 `slot` 컴포넌트와 속성을 통해서 사용할 수 있습니다. 주입해야 하는 템플릿에는 일반적인 템플릿 문법을 사용한 데이터 바인딩이 가능합니다.
